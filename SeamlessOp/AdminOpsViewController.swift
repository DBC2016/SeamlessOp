...
class AdminOpsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet private weak var opsAdminTableView     :UITableView!
    ...
    //reference code to change colors on complete audits
    ...
    //    if newEntry.winner == true {
    //    cell.backgroundColor = UIColor.greenColor()
    //    } else if newEntry.winner == false {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let destController = segue.destinationViewController as! AdminDetailViewController
        destController.currentUser = currentUser
        
        if segue.identifier == "listToDetailSegue" {
            
            let indexPath = self.opsAdminTableView.indexPathForSelectedRow
            let selectedOp = self.opsArray[indexPath!.row]
            destController.newOperation = selectedOp
            self.opsAdminTableView.deselectRowAtIndexPath(indexPath!, animated: true)
...
//            destController.newOperation = nil
    ...
    func tempAddRecords() {
        print("Temp Records Added")
                ...
                newOperation4.opsLogo = "amazonLogo"
                        ...
                        newOperation3.opBeaconID = "ice2Beacon"
    ...
    func beaconInRange(notification: NSNotification) {
        if let region = notification.userInfo?["region"] {
            ...
            opsAdminTableView.selectRowAtIndexPath(indexPath, animated: false, scrollPosition: .None)
